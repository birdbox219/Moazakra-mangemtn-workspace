...
namespace WebApplication1.Data
{
    ...
    #region Reports
    public async Task<DashboardReport>
...
)
{
    ...
    using (var cmd = new SqlCommand(
        "SELECT COUNT(*) FROM Reservation" + filter,
        conn))
    {
    ...
    {
        report.AverageReservationDuration =
            durations.Mean();

        report.MedianReservationDuration =
            durations.Median();

        report.StandardDeviationDuration =
    ...
    ", conn))
    {
        ...
        while (await reader.ReadAsync())
        {
            report.ReservationsPerMonth.Add(
                new ChartPoint
                {
                    Label =
                        reader["Label"]
